...
DEFAULT_BASE_URL = "https://TechXXX.github.io/DutchTechTestRepo/"
DEFAULT_REPO_DATA_BASE_URL = "https://raw.githubusercontent.com/TechXXX/DutchTechTestRepo/main/"
REPO_ADDON_ID = "repository.dutchtechtestrepo"
REPO_ADDON_NAME = "DutchTechTestRepo"
REPO_PROVIDER = "DutchTech"
REPO_SUMMARY = "Testing repository for DutchTech Kodi add-ons."
REPO_DESCRIPTION = (
    "Install this testing repository to receive DutchTech Kodi test add-on updates "
...
def ensure_repo_addon_source(root_dir: Path, repo_data_base_url: str) -> tuple[Path, str]:
    ...
    if not icon_path.exists() or not fanart_path.exists():
        raise SystemExit(
            "Missing repository artwork. Expected icon.png and fanart.jpg in %s" % repo_dir
        )
    ...
    for zip_path in sorted(root_dir.glob("*.zip")):
        ...
        with zipfile.ZipFile(zip_path) as zf:
            ...
            if len(addon_xml_members) != 1:
                raise SystemExit(
                    f"Expected one addon.xml in {zip_path.name}, found {len(addon_xml_members)}"
                )
...
def update_index_html(root_dir: Path, repo_version: str) -> None:
    ...
    write_text(root_dir / "index.html", "\n".join(lines) + "\n")
...
def publish_changes(root_dir: Path, repo_version: str, source_dirs: list[Path]) -> None:
    ...
    if not git_output(root_dir, "diff", "--cached", "--name-only").strip():
        ...
        return
    subprocess.run(
        ["git", "commit", "-m", f"Publish Kodi test repo {repo_version}"],
        cwd=root_dir,
        check=True,
    )
...
def main() -> None:
    ...
    _base_url = normalize_base_url(args.base_url)
