...
As of `plugin.video.fenlight.patched` `2.0.46`, playback start is intentionally
stricter:

- main startup check is about 5 seconds
- a half-started source gets a short grace settle window
- failed grace now calls `stop()` instead of blessing `started=True`

This improves zombie-start retries, but it does not fully solve the rare long
waits seen on some Real-Debrid URLs. On the problematic `UnREAL S04E04` run on
live Kodi, Fen decided the source was bad after about 8.8 seconds, then Kodi
stayed stuck in `CVideoPlayer::CloseFile()` until the RD HTTP read timed out
roughly 24 seconds later. So if a future run still feels far longer than the
configured startup timeout, inspect the teardown segment after `stop()` before
changing subtitle or selector logic.
