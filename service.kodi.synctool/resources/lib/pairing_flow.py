...
def pair_google_drive(addon=None):
    ...
    try:
        ...
        show_ok_dialog("Kodi Sync Tool", message)
        return False
    ...
    try:
        for elapsed in range(0, expires_in + interval, interval):
            ...
            if dialog.iscanceled():
                notify(addon, "Kodi Sync Tool", "Google Drive pairing cancelled")
                return False
            ...
            if state == "authorized":
                ...
                notify(addon, "Kodi Sync Tool", "Google Drive connected successfully")
                return True
            if state == "failed":
