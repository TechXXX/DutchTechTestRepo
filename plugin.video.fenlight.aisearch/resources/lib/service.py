...
pause_services_prop = 'fenlight.aisearch.pause_services'
firstrun_update_prop = 'fenlight.aisearch.firstrun_update'
current_skin_prop = 'fenlight.aisearch.current_skin'
...
class SetAddonConstants:
	def run(self):
		logger('fenlight.aisearch', 'SetAddonConstants Service Starting')
		import xbmcgui, xbmcaddon, xbmcvfs
		addon_object = xbmcaddon.Addon('plugin.video.fenlight.aisearch')
		self.window = xbmcgui.Window(10000)
		_info = addon_object.getAddonInfo
		addon_items = [('fenlight.aisearch.addon_version', _info('version')),
					('fenlight.aisearch.addon_path', _info('path')),
					('fenlight.aisearch.addon_profile', xbmcvfs.translatePath(_info('profile'))),
					('fenlight.aisearch.addon_icon', xbmcvfs.translatePath(_info('icon'))),
					('fenlight.aisearch.addon_fanart', xbmcvfs.translatePath(_info('fanart')))]
		for item in addon_items: self.set_property(*item)
		return logger('fenlight.aisearch', 'SetAddonConstants Service Finished')
...
class DatabaseMaintenance:
	def run(self):
		logger('fenlight.aisearch', 'DatabaseMaintenance Service Starting')
		from caches.base_cache import make_databases
		make_databases()
		return logger('fenlight.aisearch', 'DatabaseMaintenance Service Finished')

class SyncSettings:
	def run(self):
		logger('fenlight.aisearch', 'SyncSettings Service Starting')
...
class CustomFonts:
	def run(self):
		logger('fenlight.aisearch', 'CustomFonts Service Starting')
		...
		return logger('fenlight.aisearch', 'CustomFonts Service Finished')

class TraktMonitor:
	def run(self):
		logger('fenlight.aisearch', 'TraktMonitor Service Starting')
		...
		while not monitor.abortRequested():
			...
			try:
				...
				if status == 'failed': logger('fenlight.aisearch', trakt_service_string % ('Failed. Error from Trakt', next_update_string))
				else:
					if status in ('success', 'no account'): logger('fenlight.aisearch', trakt_service_string % ('Success. %s' % trakt_success_line_dict[status], next_update_string))
					else: logger('fenlight.aisearch', trakt_service_string % ('Success. No Changes Needed', next_update_string))# 'not needed'
					if status == 'success' and get_setting('fenlight.aisearch.trakt.refresh_widgets', 'false') == 'true': run_plugin({'mode': 'kodi_refresh'})
			except Exception as e: logger('fenlight.aisearch', trakt_service_string % ('Failed', 'The following Error Occured: %s' % str(e)))
			wait_for_abort(wait_time)
		try: del monitor
		except: pass
		try: del player
		except: pass
		return logger('fenlight.aisearch', 'TraktMonitor Service Finished')

class UpdateCheck:
	def run(self):
		window = xbmcgui.Window(10000)
		if window.getProperty(firstrun_update_prop) == 'true': return
		logger('fenlight.aisearch', 'UpdateCheck Service Starting')
		...
		return logger('fenlight.aisearch', 'UpdateCheck Service Finished')

class WidgetRefresher:
	def run(self):
		logger('fenlight.aisearch', 'WidgetRefresher Service Starting')
		from time import time
		from caches.settings_cache import get_setting
		...
		self.home = home
		self.window.setProperty('fenlight.aisearch.refresh_widgets', 'true')
		self.set_next_refresh(time())
		wait_for_abort(20)
		while not monitor.abortRequested():
			try:
				wait_for_abort(10)
				self.window.clearProperty('fenlight.aisearch.refresh_widgets')
				offset = int(self.get_setting('fenlight.aisearch.widget_refresh_timer', '60'))
				if offset != self.offset:
					self.set_next_refresh(time())
					continue
				if self.condition_check(): continue
				if self.next_refresh < time():
					run_plugin({'mode': 'refresh_widgets', 'show_notification': self.get_setting('fenlight.aisearch.widget_refresh_notification', 'false')}, block=True)
					logger('fenlight.aisearch', 'WidgetRefresher Service - Widgets Refreshed')
					self.set_next_refresh(time())
			except: pass
		try: del monitor
		except: pass
		...
		return logger('fenlight.aisearch', 'WidgetRefresher Service Finished')
...
class FenLightMonitor(xbmc.Monitor):
	...
	def onNotification(self, sender, method, data):
		if method in ('GUI.OnScreensaverActivated', 'System.OnSleep'):
			...
			logger('OnNotificationActions', 'PAUSING fenlight.aisearch Services Due to Device Sleep')
		elif method in ('GUI.OnScreensaverDeactivated', 'System.OnWake'):
			xbmcgui.Window(10000).clearProperty(pause_services_prop)
			logger('OnNotificationActions', 'UNPAUSING fenlight.aisearch Services Due to Device Awake')

logger('fenlight.aisearch', 'Main Monitor Service Starting')
FenLightMonitor().waitForAbort()
logger('fenlight.aisearch', 'Main Monitor Service Finished')
