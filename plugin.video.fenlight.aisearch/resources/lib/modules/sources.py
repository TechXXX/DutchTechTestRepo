...
int_window_prop = 'fenlight.aisearch.internal_results.%s'
scraper_timeout = 25
...
class Sources():
	...
	def playback_prep(self, params=None):
		...
		self.folders_ignore_filters = get_setting('fenlight.aisearch.results.folders_ignore_filters', 'false') == 'true'
		self.filter_size_method = int(get_setting('fenlight.aisearch.results.filter_size_method', '0'))
		...
		self.include_unknown_size = get_setting('fenlight.aisearch.results.size_unknown', 'false') == 'true'
		...
		if self.active_external or self.background:
			if self.active_external:
				if any((i[0] == 'comet' for i in self.external_providers)):
					...
					debrid_token = get_setting('fenlight.aisearch.%s' % debrid_token_dict[debrid_service])
	...
	def filter_results(self, results):
		...
		if self.filter_size_method:
			min_size = string_to_float(get_setting('fenlight.aisearch.results.%s_size_min' % self.media_type, '0'), '0') / 1000
			...
			if self.filter_size_method == 1:
				...
				max_size = string_to_float(get_setting('fenlight.aisearch.results.%s_size_max' % self.media_type, '10000'), '10000') / 1000
	...
	def special_filter(self, results, file_type):
		...
		if key == '[B]HEVC[/B]' and enable_setting == 0:
			hevc_max_quality = self._get_quality_rank(get_setting('fenlight.aisearch.filter.hevc.%s' % ('max_autoplay_quality' if self.autoplay else 'max_quality'), '4K'))
	...
	def get_folderscraper_info(self):
		folder_info = [(get_setting('fenlight.aisearch.%s.display_name' % i), i, source_folders_directory(self.media_type, i)) for i in folder_scrapers]
