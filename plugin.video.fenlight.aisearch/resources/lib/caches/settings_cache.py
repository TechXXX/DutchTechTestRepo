...
class SettingsCache:
	def get(self, setting_id):
		try:
			...
			setting_id = setting_id.replace('fenlight.aisearch.', '')
	...
	def set_memory_cache(self, setting_id, setting_value):
		set_property('fenlight.aisearch.%s' % setting_id, setting_value)

	def delete_memory_cache(self, setting_id):
		clear_property('fenlight.aisearch.%s' % setting_id)
...
def set_boolean(params):
	...
	set_setting(setting, boolean_dict[get_setting('fenlight.aisearch.%s' % setting)])

def set_string(params):
	current_value = get_setting('fenlight.aisearch.%s' % params['setting_id'])
...
def set_path(params):
	...
	new_value = kodi_dialog().browse(browse_mode, '', '', defaultt=get_setting('fenlight.aisearch.%s' % setting_id))
	set_setting(setting_id, new_value)
...
def set_source_folder_path(params):
	setting_id = params['setting_id']
	current_setting = get_setting('fenlight.aisearch.%s' % setting_id)
...
{'setting_id': 'update.location', 'setting_type': 'string', 'setting_default': 'DutchTechTestRepo'},
#==================== Watched Indicators
{'setting_id': 'watched_indicators', 'setting_type': 'action', 'setting_default': '0', 'settings_options': {'0': 'fenlight.aisearch', '1': 'Trakt'}},
#======+============= Trakt Cache
...
#==================== Downloads
{'setting_id': 'movie_download_directory', 'setting_type': 'path', 'setting_default': 'special://profile/addon_data/plugin.video.fenlight.aisearch/Movies Downloads/', 'browse_mode': '0'},
{'setting_id': 'tvshow_download_directory', 'setting_type': 'path', 'setting_default': 'special://profile/addon_data/plugin.video.fenlight.aisearch/TV Show Downloads/', 'browse_mode': '0'},
{'setting_id': 'premium_download_directory', 'setting_type': 'path', 'setting_default': 'special://profile/addon_data/plugin.video.fenlight.aisearch/Premium Downloads/', 'browse_mode': '0'},
{'setting_id': 'image_download_directory', 'setting_type': 'path', 'setting_default': 'special://profile/addon_data/plugin.video.fenlight.aisearch/Image Downloads/', 'browse_mode': '0'},
