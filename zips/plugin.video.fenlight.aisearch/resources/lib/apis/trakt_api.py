...
def call_trakt(path, params={}, data=None, is_delete=False, with_auth=True, method=None, pagination=False, page_no=1):
	def send_query():
		...
		if with_auth:
			try:
				try: expires_at = float(get_setting('fenlight.aisearch.trakt.expires'))
				except: expires_at = 0.0
				if time.time() > expires_at: trakt_refresh_token()
			except: pass
			token = get_setting('fenlight.aisearch.trakt.token')
...
def trakt_revoke_authentication(dummy=''):
	...
	CLIENT_SECRET = trakt_secret()
	if CLIENT_SECRET in empty_setting_check: return no_secret_key()
	data = {'token': get_setting('fenlight.aisearch.trakt.token'), 'client_id': CLIENT_ID, 'client_secret': CLIENT_SECRET}
...
def trakt_calendar_days(recently_aired, current_date):
	if recently_aired: start, finish = (current_date - timedelta(days=14)).strftime('%Y-%m-%d'), '14'
		...
		previous_days = int(get_setting('fenlight.aisearch.trakt.calendar_previous_days', '0'))
		future_days = int(get_setting('fenlight.aisearch.trakt.calendar_future_days', '7'))
...
def trakt_sync_activities(force_update=False):
	...
	def _check_daily_expiry():
		return int(time.time()) >= int(get_setting('fenlight.aisearch.trakt.next_daily_clear', '0'))
