...
class Navigator:
	...
	def tools(self):
		...
		if get_setting('fenlight.aisearch.external_scraper.module') not in ('empty_setting', ''):
		...
		if get_setting('fenlight.aisearch.use_viewtypes', 'true') == 'true' and not get_setting('fenlight.aisearch.manual_viewtypes', 'false') == 'true':
	...
	def changelog_utils(self):
		fenlight_clogpath = tp('special://home/addons/plugin.video.fenlight.aisearch/resources/text/changelog.txt')
	...
	def exit_media_menu(self):
		params = get_property('fenlight.aisearch.exit_params')
		if params: return container_refresh_input(params)

	def tips(self):
		tips_location = 'special://home/addons/plugin.video.fenlight.aisearch/resources/text/tips'
